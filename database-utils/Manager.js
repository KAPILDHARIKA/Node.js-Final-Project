...
const exportedMethods = {
    ...
    async getAllManager() {
        ...
        while (y < manName) {
            ...
            while (x < empName) {
                ...
                if (empo == null) throw 'Not in manager'
    ...
    async getManagerById(id) {
        ...
        while (x < empName) {
            ...
            const empo = await empCollection.findOne({ _id: ObjectId(authID) });
            if (empo === null) throw "No Manager found of following id";
    ...
    async managerExists(user_login_id) {
        if (!user_login_id) throw 'Please enter the value'
        if (typeof user_login_id != 'string') throw "Please enter correct value"
        let mCollection = await manager();

        let results = await mCollection.findOne({ user_login_id: user_login_id });
        if (results === null) throw "No Manager found of following id";
        console.log(results)
    ...
    async getManagerByUserID(user_login_id) {
        ...
        if (typeof user_login_id !== "string") throw "Please provide proper type of user_login_id"
    ...
    async addManager(firstName, lastName, email, budget, user_login_id, hashed_password) {
        ...
        if (typeof user_login_id !== 'string') throw 'provide proper user login id';
        if (typeof hashed_password !== 'string') throw 'please provide proper password';
        ...
        if (newInsertInformation.insertedCount === null) throw "No Manager found of following id";
        const newId = newInsertInformation.insertedId;
        const newManagerDetails = await this.getManagerById(newId.toString());
    ...
    async addEmptoManager(manager_ID, empId, username, total_salary, paidFlag) {
        ...
        if (!total_salary || isNaN(total_salary) || total_salary < 0) throw 'Invalid Entry4';
    ...
    async updatedManager(user_login_id, firstName, lastName, email, budget) {
        ...
        var mailformat = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!user_login_id) throw 'Please provide user login id'
        if ((!firstName) && (!lastName) && (!email) && (!budget)) throw "You must provide atleast one value to update"
        if (typeof user_login_id != 'string') throw "You must provide an  user id to search for";
        if (firstName) { if (typeof firstName !== 'string') throw 'No proper firstName provided'; }
        if (lastName) { if (typeof lastName !== 'string') throw 'No proper lastName provided'; }
        if (email) { if (mailformat.test(email) == false) throw 'Please provide proper  mailid'; }
        if (budget) { if (isNaN(budget) || (budget < 0)) throw 'Please provide proper budget'; }
    ...
    async isPaid(empId) {
        ...
        if (typeof empId != 'string') throw 'Please provide proper type empId'
        ...
        if (managerInfo.budget < (managerInfo.budget - updated.total_salary) || managerInfo.budget == 0) {
            ...
            if (updatedInfo.insertedCount === 0) {
                throw "could not update value successfully";
            }
            //adding transaction
            ...
            if (search === null) throw 'Not in Manager Collection'
